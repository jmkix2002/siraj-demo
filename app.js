...
/* =========================
   QIBLA (Real Compass + Feedback)
   ضعيه بآخر app.js
========================= */

const QIBLA_SCREEN_ID = "qibla";     // <-- عدليه لو شاشة القبلة عندك اسمها مختلف
const ARROW_ID = "qiblaArrow";       // <-- ID السهم
const KAABA_ID = "qiblaKaaba";       // <-- ID صورة/أيقونة الكعبة
...
let qiblaAngle = null;     // زاوية القبلة من الشمال (0..360)
...
function computeQiblaAngle(userLat, userLon){
...
// تحديث واجهة السهم + التنبيه
function renderQibla(){
  const screen = document.getElementById(QIBLA_SCREEN_ID);
  if (!screen || !screen.classList.contains("active")) return;

  const arrow = document.getElementById(ARROW_ID);
  const kaaba = document.getElementById(KAABA_ID);

  // حالة نصية نولدها تلقائيًا لو ما كانت موجودة
  let status = screen.querySelector(".qibla-status");
  if (!status){
    status = document.createElement("div");
    status.className = "qibla-status";
    status.textContent = "جاري تحديد القبلة...";
  ...
  const rotate = angleDiff(qiblaAngle, heading); // -180..180
  if (arrow){
  ...
  // رسالة
  status.textContent = `زاوية الانحراف عن القبلة: ${Math.round(err)}°`;
  ...
  if (err <= DEG_THRESHOLD){
    ...
    if (kaaba) kaaba.classList.remove("qibla-hit");
    didBuzz = false;
  }
}

/* ===== تشغيل GPS ===== */
function startQiblaLocation(){
  if (!navigator.geolocation){
    alert("المتصفح لا يدعم تحديد الموقع");
    return;
  }

  // Watch لتحديث إذا تغير المكان (اختياري)
  if (qiblaWatchId != null) navigator.geolocation.clearWatch(qiblaWatchId);

  qiblaWatchId = navigator.geolocation.watchPosition(
    (pos)=>{
      const lat = pos.coords.latitude;
      const lon = pos.coords.longitude;
      qiblaAngle = computeQiblaAngle(lat, lon);
      renderQibla();
    },
    ()=>{
      qiblaAngle = null;
...
function onDeviceOrientation(e){
  // iOS Safari يعطي webkitCompassHeading
  if (typeof e.webkitCompassHeading === "number"){
    heading = normalize360(e.webkitCompassHeading);
  } else if (typeof e.alpha === "number"){
    // alpha: 0..360 (لكن يعتمد على الجهاز). نطبّع فقط
...
/* ===== تشغيل تلقائي عند فتح شاشة القبلة =====
   ملاحظة iOS: إذا ما عطاك بيانات، أول لمسة داخل الشاشة بتفعل الإذن */
function initQiblaScreen(){
  startQiblaLocation();
  startQiblaCompass();
  ...
  // fallback: أول تفاعل يضمن تشغيل البوصلة على iOS
  const onFirstTap = async ()=>{
    await startQiblaCompass();
    screen.removeEventListener("click", onFirstTap);
    screen.removeEventListener("touchstart", onFirstTap);
  };
  screen.addEventListener("click", onFirstTap, { passive:true });
  screen.addEventListener("touchstart", onFirstTap, { passive:true });

  renderQibla();
}

/* ✅ اربطيها مع go() بدون تخريب
   ضعي هذا السطر داخل go(screenId) بعد تفعيل الشاشة */
(function patchGoForQibla(){
  const _go = window.go;
  if (typeof _go !== "function") return;

  window.go = function(screenId){
    _go(screenId);
    if (screenId === QIBLA_SCREEN_ID){
      initQiblaScreen();
    }
  };
})();
